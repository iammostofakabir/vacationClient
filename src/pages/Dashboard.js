...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetch(`https://morning-refuge-85756.herokuapp.com/admin/${email}`)
      .then((res) => res.json())
      .then((data) => {
        setUser(data);
        setLoading(false);
                ...
                {user?.role !== "admin" && (
                  <li className="sideBarLink">
                    <NavLink to={`${url}/myorder`}>
                      <i class="fas fa-cart-arrow-down"></i> My Posts
                    </NavLink>
                  </li>
                )}
                ...
                {user?.role === "admin" && (
                  <li className="sideBarLink">
                    <NavLink to={`${url}/addProduct`}>
                      <i class="fas fa-file-medical"></i> Add Experiences
                    </NavLink>
                  </li>
                )}
                {user?.role === "admin" && (
                ...
                {user?.role === "admin" && (
                  <li className="sideBarLink">
                    <NavLink to={`${url}/manageProduct`}>
                      <i class="fas fa-cog"></i> Manage Posts
                    </NavLink>
                  </li>
                )}
                {user?.role !== "admin" && (
